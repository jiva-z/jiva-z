<div align="center">
  <h2>👋 안녕하세요! AI & 풀스택 개발자 <b>남지윤</b>입니다.</h2>
  <p>
    <b>실질적인 가치와 혁신</b>을 만드는 프로젝트에 열정을 쏟으며 개발하고 있습니다.<br/>
    <b>"헤맨 만큼 내 땅이다"</b>라는 신념으로 한 줄 한 줄 쌓아, 오늘도 저만의 개발 지도를 확장하고 있습니다.
  </p>
</div>

<br/>

---

## 🚀 주요 프로젝트


### 👷 [산재 대체 인력 매칭 AI 플랫폼, 급구당](https://web-workekmanagers-fe-m9i2iiuc1e546d59.sel4.cloudtype.app/)
> **AI 기반 실시간 대체 인력 매칭 시스템**  
> <sub>2025.05 / PM & Full Stack Developer & DevOps</sub>

- **한 줄 소개:**  
  산재 발생 시 AI가 요양기간을 예측하고, 대체 인력 매칭을 수행하는 AI 플랫폼 개발

- **주요 역할 및 성과:**
  - **전체 시스템 아키텍처 설계**
    - Spring Boot + React + FastAPI 조합의 MSA 구조 채택
  - **핵심 기능 개발**
    - JWT 기반 인증 시스템 구현
    - 요양기간 예측 API 설계 및 개발 : 응답 시 300ms 내 처리
    - 북마크 기능: 사용자별 채용공고 저장 시스템 구현
    - FastAPI 기반 AI 모델 서버와 Spring Boot 연동을 위한 RESTful API 구축
  - **클라우드 배포**
    - Cloudtype 환경에 Docker 컨테이너 기반 Spring Boot, React, MySQL, FastAPI 배포
      
#### 🌐 **Live Demo**
https://github.com/user-attachments/assets/f27e7487-a738-4820-bd18-cbcdeae5721b

<br/>

### 🌱 네이처 히어로즈(Nature Heroes) 기획
> **시민 참여형 생태 경험 앱**  
> <sub>2025.03 / PM & 서비스 기획</sub>

- **한 줄 소개:**
  게임적 재미와 교육적 가치를 결합해, 시민이 자연과 교감하고 환경 보호에 참여할 수 있는 체험형 생태 플랫폼 기획
- **주요 역할 및 성과:**
  - **프로젝트 PM 및 서비스 기획 총괄**
  - 시민참여형 생태 경험 서비스 아이디어 도출 및 핵심 기능 정의
  - 공공데이터(서울시 야생동식물, 서식지 등) 및 위치 기반 서비스 연계 방안 설계
  - 게임화(랭킹, 퀴즈, 꾸미기 등) 요소 기획 및 UX 플로우 설계
  - 교육·공공기관 협력 및 확장성, 사업성 분석

<br/>

### 🏠 [안주: AI 결함 탐지 및 자동 보고서 생성을 통한 노후주택 관리 시스템](https://github.com/HomePPlus/safehouse.git)
> **AI가 만드는 편안한 안식처 "안주"**  
> <sub>2025.02 / PL & Tech Lead</sub>

- **한 줄 소개:**  
  YOLO·LangChain 기반 AI 결함 탐지 및 자동 안전점검 보고서 시스템 개발
- **주요 역할 및 성과:**
  - 서비스 비전 및 단계별 로드맵 수립, 전체 기획 산출물 작성
  - Inspection 도메인 중심 RESTful API, 중복 예약 방지, 상태 전이 검증
  - FastAPI 기반 YOLO 딥러닝 모델 연동, 이미지 결함 자동 탐지/신고 파이프라인 구축
  - JPA 기반 ORM/트랜잭션 관리(데이터 오류·중복 95%↓), 글로벌 예외 처리
  - React 기반 Atomic Design UI, JWT 인증/인가, Protected Route 구현
  - Three.js 등 애니메이션 기반 라이브러리 활용으로 대시보드·보고서 시각화, 사용자 인터랙션을 강화한 프론트엔드 개발

#### 🌐 **Live Demo**
https://github.com/user-attachments/assets/d4137069-141f-4230-8ec7-24a7c282a0c7

<br/>

### 🏥 [다국어 응급상황 인식 및 응급실 추천 AI 서비스](https://github.com/jiva-z/Recommend-Emergency-Room.git)
> **다국어 음성 인식 & 응급실 추천 시스템**  
> <sub>2024.12 / AI & Full Stack Developer</sub>

- **한 줄 소개:**  
  한·영 음성 데이터로 응급상황 자동 인식, 위치 기반 최적 응급실 추천
- **주요 역할 및 성과:**
  - 음성 데이터 전처리, STT, NLP 요약, BERT 커스텀 분류기 파인튜닝
  - 멀티랭귀지(한국어/영어) 지원, 성능 개선 및 모델 최적화
  - Spring Security 인증/권한, FastAPI AI 서버 비동기 연동(3초 이내 응답)
  - KTAS 기반 응급실 추천 알고리즘(정확도 45%↑), 위치 데이터베이스 구축
  - Mustache 기반 SSR, 반응형 UI, 실시간 데이터 바인딩, 사용자 피드백 컴포넌트 구현

---

<br/>


## 🛠️ 기술 스택

<div align="center">
  <img alt="Python" src="https://img.shields.io/badge/Python-3776AB.svg?&style=flat&logo=Python&logoColor=white"/>&nbsp;
  <img alt="Java" src="https://img.shields.io/badge/Java-007396.svg?style=flat&logo=OpenJDK&logoColor=white"/>&nbsp;
  <img alt="JavaScript" src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=JavaScript&logoColor=black"/>&nbsp;
  <img alt="R" src="https://img.shields.io/badge/R-276DC3.svg?&style=flat&logo=R&logoColor=white"/>&nbsp;
  <img alt="HTML5" src="https://img.shields.io/badge/HTML5-E34F26.svg?&style=flat&logo=HTML5&logoColor=white"/>&nbsp;
  <img alt="CSS" src="https://img.shields.io/badge/CSS-663399.svg?&style=flat&logo=CSS&logoColor=white"/>&nbsp;
  <img alt="SPRING" src="https://img.shields.io/badge/SPRING-6DB33F?style=flat&logo=SPRING&logoColor=white"/>&nbsp;
  <img alt="React" src="https://img.shields.io/badge/React-61DAFB?style=flat&logo=React&logoColor=black"/>&nbsp;
  <img alt="DOCKER" src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=Docker&logoColor=white"/>&nbsp;
  <img alt="MySQL" src="https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=MySQL&logoColor=white"/>&nbsp;
  <img alt="Qgis" src="https://img.shields.io/badge/Qgis-589632?style=flat&logo=Qgis&logoColor=white"/>&nbsp;
</div>

---

<br/>

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=jiva-z&show_icons=true&theme=dracula" width="400" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs?username=jiva-z&layout=compact&langs_count=10&bg_color=45,654ea3,eaafc8&title_color=ffffff&text_color=ffffff&hide_border=true" width="360" />
</div>

<br/>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=jiva-z&theme=dracula&hide_border=true&radius=8" width="98%" />
</div>

---

<br/>

## 🏅 Coding Rank

<div align="center">
  <kbd>
    <img src="https://raw.githubusercontent.com/jiva-z/Programmers_Badge_Generator/main/result/result.svg" width="551px"/>
  </kbd>
</div>

---

<br/>
